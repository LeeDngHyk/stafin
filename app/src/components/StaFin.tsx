  ...
  return (
    <Animated.View style={[{ width: size, height: size }, aStyle]}>
      <Svg width={size} height={size} viewBox="0 0 100 100">
        ...
        <Polygon points={starPoints(50, 52, 46, 20)} fill="url(#star)" stroke="#000000" strokeWidth={1.5} strokeLinejoin="round" />

        {/* 볼터치 (모노크롬: 미세 하이라이트) */}
        <Circle cx={33} cy={58} r={5} fill="#FFFFFF" opacity={0.08} />
        <Circle cx={67} cy={58} r={5} fill="#FFFFFF" opacity={0.08} />

        {/* 눈 */}
        {eyeShape === "spark" ? (
          <>
            <Path d="M38 44 L41 50 L44 44 L41 38 Z" fill="#FFFFFF" />
            <Path d="M56 44 L59 50 L62 44 L59 38 Z" fill="#FFFFFF" />
          </>
        ) : eyeShape === "soft" ? (
          <>
            <Path d="M36 48 Q41 43 46 48" stroke="#FFFFFF" strokeWidth={2.4} fill="none" strokeLinecap="round" />
            <Path d="M54 48 Q59 43 64 48" stroke="#FFFFFF" strokeWidth={2.4} fill="none" strokeLinecap="round" />
          </>
        ) : (
          <>
            <Ellipse cx={41} cy={lookUp ? 45 : 48} rx={4} ry={5} fill="#FFFFFF" />
            <Ellipse cx={59} cy={lookUp ? 45 : 48} rx={4} ry={5} fill="#FFFFFF" />
            <Circle cx={42.4} cy={lookUp ? 43.4 : 46.4} r={1.6} fill="#1B1C1C" />
            <Circle cx={60.4} cy={lookUp ? 43.4 : 46.4} r={1.6} fill="#1B1C1C" />
          </>
        )}

        {/* 입 */}
        {mood === "celebrate" || mood === "happy" || mood === "wave" ? (
          <Path d="M42 56 Q50 64 58 56" stroke="#FFFFFF" strokeWidth={2.6} fill="none" strokeLinecap="round" />
        ) : mood === "think" ? (
          <Circle cx={50} cy={58} r={2.2} fill="#FFFFFF" />
        ) : (
          <Path d="M44 57 Q50 61 56 57" stroke="#FFFFFF" strokeWidth={2.4} fill="none" strokeLinecap="round" />
        )}
      </Svg>
